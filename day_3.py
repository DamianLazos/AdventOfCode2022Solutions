...
dataset = '''shzsFcPssFhjFssBzdpRcNHNZrpdJdJVJZ
...
def organizeData(dataset) -> list:
    '''This functions creates a list with
    every string of items removing white
    spaces'''
    splitByRows = dataset.split("\n")
    data = [row for row in splitByRows if row != '']
    return data

def distributeItems(data) -> list:
    '''This funtion takes a list of strings
    and distribute by half each one finding
    the common letter between them.'''
    
    itemsProportions = []
    
    for row in data:
        itemsCount = sum(1 for letter in row if letter.isalpha())
        proportion = int(itemsCount / 2)
        leftProportion = row[:proportion]
        rightProportion = row[proportion:]
        intersection = ''.join(set(leftProportion).intersection(set(rightProportion)))
        itemsProportions.append((row, leftProportion, rightProportion, intersection))
    return itemsProportions

def lettersValues() -> dict:
    '''This function creates a single
    dictionary with every letter of the 
    alphabet with its regarding position.
    Upper letters starts from the position
    27'''
    import string
    mapAlphaLow = dict(zip(string.ascii_lowercase, range(1, 27)))
    mapAlphaUp = dict(zip(string.ascii_uppercase, range(27, 53)))
    mapAlphaLow.update(mapAlphaUp)
    return mapAlphaLow

def getLetterValue(letter:str, mapAlpha) -> int:
    '''This function finds the corresponding
    value of a given letter from a dictionary
    of letters and values'''
    value = mapAlpha.get(letter, "Nothing")
    return value

data = organizeData(dataset=dataset)
itemsProportions = distributeItems(data=data)
mapeado = lettersValues()

counter = sum(mapeado[rucksack[3]] for rucksack in itemsProportions)
print(counter)
...
while len(itemsProportions) != 0:
    rucksack1, rucksack2, rucksack3 = itemsProportions[0:3]
    print(rucksack1[0], rucksack2[0], rucksack3[0])
    commonLetter = ''.join(set(rucksack1[0]).intersection(set(rucksack2[0]), set(rucksack3[0])))
    itemsProportions = [item for index, item in enumerate(itemsProportions) if index not in [0, 1, 2]]
    total += getLetterValue(letter=commonLetter, mapAlpha=mapeado)
    rucksacksCounter += 1
print(total)
